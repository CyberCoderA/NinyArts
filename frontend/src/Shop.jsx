...
function Shop() {
  ...
  const cartItems = itemData
    .map((item, index) => (addedItems[index] ? { ...item, index } : null))
    .filter(Boolean);
    ...
    <div className="bg-white h-screen w-full flex flex-col items-center">
      ...
      <div className="h-full w-full flex flex-col justify-around items-center gap-10 p-10 lg:justify-start">
        <div className="flex flex-col gap-2">
          <h1 className="font-bold text-center text-5xl text-primary">Shop</h1>
          ...
          <div className="flex flex-col w-full gap-5 lg:flex-row">
            {itemData.map((item, index) => (
              <ProductCard
                ...
                title={item.title}
                image={item.image}
                added={addedItems[index]}
                onToggleCart={() => toggleCart(index)}
              />
            ))}
          </div>
        </div>
        ...
        <div className="fixed bottom-4 right-4">
          <div
            className="relative w-30 h-30 rounded-full bg-primary flex justify-center items-center shadow-2xl cursor-pointer"
      ...
      {isCartOpen && (
        <>
          <div
            className="fixed inset-0 bg-accent opacity-80 backdrop-blur-sm z-40"
            onClick={toggleCartPopup}
          ></div>
